...
geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Average Faculty Salary", 
       ...
       title = "Income of Students based on 
	Average Faculty Salaries at their University")
```
...
geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Average SAT Score",  
	y = "2011 Median Income of Students",
       title = "Income of Students based on 
	Average SAT Scores at their University")
```
...
First, the intercept value is -2590.98. This intercept represents the earnings for an individual who has sat score of and their faculty's income is $0, meaning they are not in college.

Second, for every increase of one dollar in faculty earnings, there is an associated increase of on average $24.48 in the earnings of the student.

Third, taking into account all other explanatory variables in our model, for every increase of one point in SAT scores, there is an associated increase of, on average, $0.38 in student's earnings.
...
Applying the Rubin Causal Model to this dataset:

Unit: School/college

Treatment: Cutting the financial aid budget (or average SAT score)

Outcomes: Earnings of students 10 years after they enter college

Fundamental problem of causal inference: We cannot know the outcomes for each university under each treatment, because only one option can actually occur.

In order for this to be a causal claim, the treatment should be randomized to minimize confounding factors.
